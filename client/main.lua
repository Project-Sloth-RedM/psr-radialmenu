...
local function setRadialState(bool, sendMessage, delay)
    ...
    if bool then
        TriggerEvent('psr-radialmenu:client:onRadialmenuOpen')
        SetupRadialMenu()
    else
        TriggerEvent('psr-radialmenu:client:onRadialmenuClose')
    end
...
CreateThread(function()
    while true do
		Wait(7)
		if IsControlJustPressed(0, PSRCore.Shared.Keybinds['F6']) then --F6  
...
RegisterNetEvent('PSRCore:Client:OnPlayerLoaded', function()
    PlayerData = PSRCore.Functions.GetPlayerData().job
end)
...
RegisterNetEvent('psr-radialmenu:client:noPlayers', function()
    PSRCore.Functions.Notify('No One Nearby', 'error')
end)
...
RegisterNUICallback('selectItem', function(inData, cb)
    ...
    if data and found then
        ...
        elseif data.type == 'psrcommand' then
            TriggerServerEvent('PSRCore:CallCommand', data.event, data)
        end
    end
    cb('ok')
end)
